...
class MedikamentDBApiController extends AbstractController
{
  ...
  public function apiPost(
    ...
    if ($event === 'sendUdstyrListeInfo') {
      $base64_string = $request->getContent(); // Get the base64 string from the request body
      $data = base64_decode($base64_string); // Decode the base64 string to get the JSON data
      $data = json_decode($data, true); // Decode the JSON data into an associative array

      // Check if the userId is valid
      $user = $entityManager->getRepository(User::class)->find($userId); // Find the user by ID
      if (!$user) {
